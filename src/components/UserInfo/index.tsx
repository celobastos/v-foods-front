import styles from './index.module.css';

interface UserInfoProps {
    picture: string;
    name: string;
    cargo: string;
    email: string;
    cellphone: string;
}

const UserInfo = ({picture, name, cargo, email, cellphone}: UserInfoProps) => {
  return (
    <div>
        <div className={styles['profile']}>
            <div className=' flex mt-[21px] '>
                <div className='absolute left-[353px]'>
                    <img src={picture} className=' w-28 h-28 rounded-full'></img>
                </div>

                <div className=' ml-44'>
                    <div className='border-b border-gray-300 pb-4'>
                        <h1 className=' text-3xl font-bold'>{name}</h1>
                        <p className=' text-gray-500 font-semibold text-sm'>{cargo}</p>
                    </div>
                    <div className='flex'>
                        <div className=' mt-4'>
                            <p className=' font-medium flex text-sm'><img src='/images/mail.png' className=' w-[18px] h-[14px] mr-2 mt-[2px]'></img>Email</p>
                            <p className='text-gray-500 text-sm mt-1'>{email}</p>

                        </div>
                        <div className=' mt-3 ml-8'>
                            <div className='flex'>
                            <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>
                            <div className=' font-medium flex text-sm m-1'>Telefone</div>

                            </div>
                            <p className='text-gray-500 text-sm'>{cellphone}</p>

                        </div>
                    </div>
                    
                </div>
            </div>
        </div>
    </div>
  )
}

export default UserInfo